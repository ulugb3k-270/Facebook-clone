...
export default function Post({
  ...
  const [sendGifs, setSendGifs] = useState(false);
  ...
  const postComment = async (url) => {
    ...
    if (url) {
      ...
      await uploadString(imageRef, url, "data_url").then(async (snapshot) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const data = await axios("https://api.giphy.com/v1/gifs/trending", {
        params: {
          api_key: "7ilX0lxr6H1SCLHL2CUBGs6VBgyGCGsI",
        },
      });
      ...
      {!commentBool && (
        <>
          ...
          <form className="p-3 flex justify-between gap-2 items-center">
            ...
            <div className="flex-1 flex items-center bg-[#f0f2f5] rounded-full px-3 relative">
              {sendGifs && (
                <div className="absolute  w-[340px]  -top-[260px] right-0 bg-[#f0f2f5] p-2 rounded-md">
                  <form
                  ...
                  >
                    <input
                      type="text"
                      className="rounded-full flex-1 px-2 py-1 focus:outline-none"
                      placeholder="Search Gif"
                    />
                    <button type="submit" onClick={searchGif}>
                      <SearchOutlined />
                    </button>
                  </form>
                  <div className="flex flex-wrap gap-1 overflow-y-scroll max-h-[200px] h-full">
                    {gifs.map((gif) => (
                      <img
                        key={gif.id}
                        src={gif.images.preview_gif.url}
                        alt={gif.username}
                        className="max-w-[100px] w-full object-cover cursor-pointer"
                        onClick={() => postComment(gif.images.preview_gif.url)}
                      />
                    ))}
                  </div>
                </div>
